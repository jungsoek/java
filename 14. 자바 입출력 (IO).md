...
`InputStream`은 **바이트 단위**이므로 텍스트를 읽으려면 문자 변환이 필요해:
...
> `java.io.File` 클래스는 **파일 자체를 다루는 게 아니라**, 경로(path) 정보를 다루는 **"파일 시스템 추상화 객체"**야.
...
> `java.nio.file.Files`는 자바 7부터 추가된 **정적 메서드 중심 유틸리티 클래스**야.
>  `Path` 객체와 함께 사용하며, **파일을 한 번에 읽거나 쓰기**에 매우 편리해.
