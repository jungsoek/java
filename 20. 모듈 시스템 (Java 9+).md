...
`module-info.java`는 **모듈 선언 파일**로, Java 9 이상의 모듈 시스템(JPMS: Java Platform Module System)을 사용하는 프로젝트의 최상위에 위치함.
 이 파일을 통해 모듈의 **의존성**, **공개 API**, **서비스 제공 및 사용** 등을 선언할 수 있어.
...
`exports`는 **현재 모듈이 외부에 공개하고자 하는 패키지를 지정**할 때 사용해.
 `exports`하지 않으면, **public 클래스여도 외부에서 접근할 수 없어.**
...
이 경우, `com.example.api` 패키지 안의 public 클래스들만 다른 모듈에서 사용할 수 있어.
...
`requires`는 **현재 모듈이 동작하는 데 의존하는 외부 모듈을 명시**할 때 사용해.
 자바 모듈 시스템에서는 클래스 경로가 아니라 **모듈 이름**으로 의존성을 정의해.
...
**한 모듈이 다른 모듈의 public API에 접근하기 위해 선언하는 관계**야.
 이 의존 관계는 `requires`, `requires transitive`, `exports`, `exports to` 같은 키워드로 설정돼.
...
Java 모듈 시스템은 **모듈 간의 순환 의존을 허용하지 않아.**
...
→ 이런 경우는 **인터페이스 분리** 또는 **공통 인터페이스 모듈**을 만들어 해결해야 해.
