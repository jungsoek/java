...
Java 9+부터는 `module-info.java`를 기반으로 **명시적 의존성**과 **모듈화**가 가능해졌어.
 예시:
...
`jlink`는 **자바 런타임 이미지(JRE)** 를 내 애플리케이션에 **최소화된 형태로 포함**시킬 수 있게 해줘.
 즉, **필요한 모듈만 포함한 JRE 생성**이 가능해서 전체 크기가 작아지고 성능도 최적화돼.
...
`jpackage`는 JDK 14+에서 제공되고, Windows/Mac/Linux별 **실행파일(EXE, DMG, DEB, RPM 등)** 을 만드는 데 사용해.
